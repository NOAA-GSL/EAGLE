...
- The `checkpoint:` parameter specifies the location of the training checkpoints, to be used in by the training step.
...
- The `remove:` block specifies values from the default Anemoi configuration that should be removed at execution time, via the [override syntax](https://hydra.cc/docs/advanced/override_grammar/basic/#basic-override-syntax) of [Hydra](https://hydra.cc/), the YAML-processing tool used by Anemoi.
